...
describe("Original Token staking", async () => {
    ...
    it("Create test regular Token to stake", async () => {
        ...
        user1StakeAta = await getAssociatedTokenAddress(stakingTokenMint, payer.publicKey, false, TOKEN_PROGRAM_ID)
    ...
    it("Create test stake pool with original Tokens!", async () => {
    ...
    it("Stake tokens!", async () => {
        ...
        let userTokenAcct = await getAccount(provider.connection, user1Ata, undefined, TOKEN_PROGRAM_ID)
        ...
        let stakeVaultAcct = await getAccount(provider.connection, stakeVault, undefined, TOKEN_PROGRAM_ID)
